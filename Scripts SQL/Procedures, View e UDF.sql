...
begin
	...
	BEGIN
		...
		begin
			...
			if(@media_final < 6 and @media_final > 6 and (select exame from @alunos_media where ra_aluno = @Ra) is null)
			begin
				update @alunos_media set situacao = 'Exame' where ra_aluno = @Ra	
			end
		
			if(@media_final < 6 and (select exame from @alunos_media where ra_aluno = @Ra) is  not null)
			begin
